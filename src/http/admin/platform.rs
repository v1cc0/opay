...
use crate::{
    ...
    platform::PlatformSearchUser,
};

pub fn router() -> Router<AppState> {
    Router::new()
        .route("/api/admin/platform/groups", get(get_groups))
        .route("/api/admin/platform/search-users", get(search_users))
}

#[derive(Debug, Deserialize)]
struct AdminPlatformQuery {
    token: Option<String>,
    lang: Option<String>,
}
...
struct UsersResponse {
    users: Vec<PlatformSearchUser>,
}

async fn get_groups(
    State(state): State<AppState>,
    headers: HeaderMap,
    Query(query): Query<AdminPlatformQuery>,
) -> AppResult<Json<GroupsResponse<crate::platform::PlatformGroup>>> {
    verify_admin_values(
        &headers,
    ...
    let platform = state
        .platform
        .as_ref()
        .ok_or_else(|| AppError::public_internal("获取 Platform 分组列表失败"))?;
    let admin_api_key = platform_admin_api_key(&state).await?;
    let groups = platform
        .get_all_groups(&admin_api_key)
        .await
        .map_err(|_| AppError::public_internal("获取 Platform 分组列表失败"))?;
