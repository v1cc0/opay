...
use crate::{
    ...
    platform::{ListSubscriptionsParams, PaginatedSubscriptions, PlatformSubscription},
};
...
async fn get_subscriptions(
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        let (subscriptions, user) = tokio::try_join!(
            platform.get_user_subscriptions(parsed_user_id, &admin_api_key),
            async {
                match platform.get_user(parsed_user_id, &admin_api_key).await {
...
mod tests {
    ...
    async fn test_state(platform_base_url: Option<String>) -> AppState {
        let db_path =
            std::env::temp_dir().join(format!("opay-admin-subs-route-{}.db", Uuid::new_v4()));
